...
const MegaMenuIndustries = () => {
  const industries = [
    { icon: Heart, title: 'Healthcare', href: '#healthcare' },
    { icon: Banknote, title: 'Finance', href: '#finance' },
    { icon: ShoppingCart, title: 'Retail', href: '#retail' },
    { icon: Factory, title: 'Manufacturing', href: '#manufacturing' },
    { icon: Plane, title: 'Telecommunications', href: '#telecommunications' },
    { icon: Building2, title: 'Information Technology', href: '#it' },
    { icon: Banknote, title: 'BFSI', href: '#bfsi' },
    { icon: TrendingUp, title: 'Engineering', href: '#engineering' },
    { icon: Factory, title: 'Renewable Energy', href: '#renewable-energy' },
    { icon: Factory, title: 'Oil & Gas', href: '#oil-gas' }
  ];

  return (
    <div className="grid grid-cols-12 gap-6 p-8 bg-gray-50 rounded-xl shadow-md mt-[-30px] w-full"> {/* Increased padding and width */}
      
      {/* Left Navigation */}
      <div className="col-span-4 space-y-4"> {/* Increased column span */}
        <h3 className="text-md font-bold text-gray-900 mb-2 border-b pb-1">Industries</h3>
        <div className="grid grid-cols-2 gap-4"> {/* Improved spacing */}
          {industries.map((industry) => (
            <a key={industry.title} href={industry.href} className="flex items-center p-4 bg-white rounded-md border border-gray-200 hover:border-blue-400 shadow-sm transition-all">
              <div className="p-2 bg-blue-100 rounded-md">
                {React.createElement(industry.icon, { className: 'h-6 w-6 text-blue-600' })} {/* Increased icon size */}
              </div>
              <h4 className="text-sm font-semibold text-gray-900 ml-3">{industry.title}</h4> {/* Adjusted spacing */}
            </a>
          ))}
        </div>
      </div>
      ...
      <div className="col-span-5 bg-white rounded-xl p-6 shadow-sm">
        ...
        <img src="https://images.pexels.com/photos/4386466/pexels-photo-4386466.jpeg?auto=compress&cs=tinysrgb&w=600" alt="Industry insights" className="w-full h-44 object-cover rounded-lg shadow-sm mb-4" /> {/* Increased image size */}
        ...
        <div className="mt-4 flex items-center justify-between">
          <a href="#enterprise" className="inline-flex items-center bg-blue-600 text-white px-5 py-3 rounded-md font-semibold hover:bg-blue-700 transition-all">
